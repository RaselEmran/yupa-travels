...
class CheckoutController extends Controller {
	...
	public function post_pack_checkout(Request $request) {
		...
		if ($check->pack_quantity != $request->guest || $check->total != $request->total) {
			...
			$details = new Details();
			$details->setSubtotal($request->total / config('settings.c_rate', config('system.defualt_curency_rate')));

			$amount = new Amount();
			$amount->setCurrency("USD")
				->setTotal($request->total / config('settings.c_rate', config('system.defualt_curency_rate')))
				->setDetails($details);
	...
	public function post_hotel_checkout(Request $request) {
		...
		if ($check->guest != $request->guest || $check->price != $request->price) {
			...
			$payer->setPaymentMethod("paypal");
			$details = new Details();
			$details->setSubtotal($request->price / config('settings.c_rate', config('system.defualt_curency_rate')));

			$amount = new Amount();
			$amount->setCurrency("USD")
				->setTotal($request->price / config('settings.c_rate', config('system.defualt_curency_rate')))
				->setDetails($details);
